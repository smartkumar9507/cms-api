...
// get a specific student by its enrollmentNo 
...
const addDetails = async (req, res) => {
    try {
        ...
        user = await studentDetails.create({ ...req.body, profile: req.body.profile });
        const data = {
            success: true,
...
const getAllStudents = async (req,res)=>{
    try {
        ...
        if(!students) return res.status(404).json({status:success, message:"Student Not Added!"})
...
module.exports = { getDetails, addDetails, updateDetails, deleteDetails, getCount, getAllStudents }